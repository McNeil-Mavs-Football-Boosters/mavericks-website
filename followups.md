...
- [ ] **Build `/sponsors` page** per `content_map_v2.md` lines 292–331. Tier-grouped public list (MVP largest → Blue smallest, hide-if-no-sponsors-at-this-tier rule, footer CTA card). Header link, mobile-nav link, footer link, homepage "See all sponsors →" link, and the new `Become a Sponsor` carousel CTA's sibling all 404 today. Seed data in place since 2026-05-22 (migration 041). Read logos via `publicStorageUrl(logo_url, 'sponsor-logos')`; wrap each tier logo in `<a target="_blank" rel="noopener noreferrer">` to its `website_url` per spec line 423.
