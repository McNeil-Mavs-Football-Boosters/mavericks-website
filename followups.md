...
- [ ] **Build `/boosters/volunteer` page** — existing carousel CTA already targets this. 404 today. **Two CTAs now point here** from `/boosters/committees` (top-hero green button + bottom card "Volunteer with the Mavs"). Higher visibility = higher priority. Natural next page after committees.
...
- **`BOOSTER_LINKS` is duplicated** in `components/layout/Header.tsx:15-25` and `components/layout/MobileNav.tsx:22-32`. Same content, both surfaces. 2026-05-23 ("Join" → "Join the Club!", Board removed) had to edit both files identically — flagged in commit `c7668d6` report. Lift to `components/layout/teamLinks.ts` (already houses shared `NavLink` + roster/schedule link builders) at the next nav touch to prevent drift. Same likely applies to the schedule/roster nav constants if they're also duplicated — check then.
