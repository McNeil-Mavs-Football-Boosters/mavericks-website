...
| Schedule | Routes for `/schedule`, `/schedule/games/[level]`, `/schedule/games/freshman/[designation]`, `/schedule/practice/[level]`; shared sticky tab nav |
| Roster | Routes for `/roster`, `/roster/[level]`, `/roster/freshman/[designation]`; shared sticky tab nav; player table render |
...
- `freshman_has_blue` is read in routes that conditionally render Blue tabs/pages. See Section 5.
...
1. **Page header**
   - Title: `[current_year] [Level] [Games|Practice]` (e.g., "2026-27 Varsity Schedule", "2026-27 Freshman Practice")
   - For game pages only: subhead with MaxPreps CTA `"Live scores and stats →"` opening `site_settings.maxpreps_team_url` in a new tab

2. **Sticky secondary nav** (three rows; remains visible on scroll)
   - **Type row:** `Games | Practice`
   - **Level row:** `Varsity | JV | Freshman`
   - **Designation row:** only renders when the current Type+Level combination has multiple designations available:
     - For Games + Freshman: always renders Green; renders Blue when `freshman_has_blue = true`
     - For Practice + any level: never renders (practice has no designation per addendum 3)
     - For Games + Varsity/JV: never renders

The active tab on each row should be visually distinct. Clicks navigate to the corresponding route.
...
**Empty state:** card showing "[Level] [designation if any] schedule coming soon. Check MaxPreps for current details." with a MaxPreps button.
...
2. **Sticky tab nav**
   - **Level row:** `Varsity | JV | Freshman`
   - **Designation row:** only when on Freshman; Green always, Blue when `freshman_has_blue = true`
...
- All tab navs are keyboard-navigable (arrow keys to move between tabs, Enter to activate)
- Sticky tab nav announces active state to screen readers (`aria-current="page"`)
...
Header nav from Step 4b already points to `/schedule`, `/roster`, `/coaches`, and `/resources` (label "For Parents & Athletes"). Verify all four resolve after Commit B. The other top-level nav items (`/news`, `/sponsors`, `/boosters/*`) remain 404 or partial; that's fine and explicitly out of scope.
...
- Hides: header nav, footer, sticky tab nav, the Print button itself, the MaxPreps button, any other action buttons
- Shows: page title (rendered as a real h1 with the year + level), the data (games table, roster table, or practice markdown body), a small footer line with the page URL and the print date
...
- [ ] Toggling `freshman_has_blue = true` in DB and revalidating: blue tab appears in nav, `/schedule/games/freshman/blue` and `/roster/freshman/blue` render
...
## 12. Decisions (locked 2026-05-16)

| # | Decision | Resolution |
|---|---|---|
| 1 | Caching strategy | Dynamic. No `revalidate` directive. Fresh data every request. |
| 2 | NULL freshman row handling | Strict. No render-layer fallback. Admin UI enforces the field at entry. |
| 3 | Print support | Print button + print stylesheet on schedule (games + practice) and roster pages. Coaches and resources excluded. |
| 4 | Home-game visual treatment | Apply light background tint to home-game rows. |
| 5 | Height/weight on mobile roster | Show all columns. The sideline use case justifies the third row. |
| 6 | `getCurrentYear()` helper location | Builder's call. Recommend `lib/site-settings.ts` returning the full settings object. |
| 7 | Build order within Commit B | A (current_year) → B (Schedule) → C (Roster) → D (Coaches) → E (Resources), one commit per slice. |
...
Total: ~3 evenings if everything lines up. Slack room for one round of "this doesn't render right on mobile" rework. If it stretches past 4 evenings, scope creep is happening; stop and re-spec.
