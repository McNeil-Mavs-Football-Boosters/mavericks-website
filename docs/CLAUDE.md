...
## Where things stand (read this first — updated 2026-09-22)

**2026-09-22 (late) — Coach emails + teaching subjects from the McNeil staff directory (migration 214). Last migration applied: 214.** Directory at `mcneil.roundrockisd.org/o/mcneil/staff` read with headless Chromium (use `wait_until='domcontentloaded'`; `networkidle` times out) and searched by last name via the `Search` placeholder input. **All 12 active coaches now carry an RRISD email** (new: Gillis, Matthews, Edwards as `nicholas_edwards@`, Umberger, Texada). Teaching lines added: Matthews CTE, Edwards Social Studies, Umberger + Texada PE. ⚠️ **Debose changed Special Education → CTE** (directory now lists him under CTE). Gillis's department is just "Athletics", so no teaching line. **The directory lists no phone numbers for coaches**; `phone` untouched and asserted null. Verified live on `/coaches`. `apply_all.sql` 216/216.

**2026-09-22 (late) — Photo-day headshots for all 12 coaches + Raleigh Texada added (migration 213). Last migration applied: 213.** Jeremy supplied 12 portraits named by coach. Cropped by `MavericksWebsite/coach_photos/crop_photoday_2026.py` from pinned `sources/photoday-2026/` (600x600, same method/guards as `crop_faces.py`; Haar failed on Debose and Gardner, pinned hand boxes; all reviewed on a contact sheet). Uploaded as NEW objects `coach-photos/Coach<Name>2026.jpg` (31-day image cache, never overwrite); old objects left in the bucket, `213_rollback.sql` restores the old URLs captured from the live rows. **Doyle had no photo before; now every active coach has one** (verify block asserts it). **Raleigh Texada** inserted: Defensive Backs Coach, `position_coach`, sort 36. ⚠️ **Devonte Jones is ALSO still "Defensive Backs Coach"** — Jeremy said Texada is the DB coach and said nothing about Jones; confirm whether Jones moved. Verified live on `/coaches`: 12 `…2026.jpg` images, no old photo URLs, Texada listed. `apply_all.sql` 215/215.

**2026-09-22 (late) — Yard sign order form on /resources (migration 212). Last migration applied: 212.** `2026 Player Yard Sign ($30)` in Registration & Forms after the meal program, linking the `/viewform` URL (not `/edit`). ⚠️ The form's own header still says "Order deadline is Friday, September 8, 2026" (past, and Sep 8 is a Tuesday); ordering is live because Ashley places the combined senior + yard sign order Thu/Fri Sep 24-25 per the 9/22 minutes. No deadline on the site until someone sets one. Newsletter promotion: Jeremy is sending an issue 9/23 himself; nothing built here. `apply_all.sql` regenerated.

**2026-09-22 (late) — Cane's Oct 26 fundraiser page wired in (migration 211). Last migration applied: 211.** Raising Cane's created `raisingcanescommunity.com/event/ccbc5242-…` for the Oct 26 night; Jeremy: "how can we add this?" Read with headless Python Playwright (Cloudflare 403s curl). Now the Oct 26 event's `signup_url` with `signup_label` = **"Fundraiser Details"** (navy button on `/events/[slug]`; "Sign Up" would misdescribe it). Three facts from the page changed the row: ⚠️ **Cane's says 5–9 PM, Kendra said 5–8** — site follows the restaurant (what the register honours), `ends_at` → 9:00, prose moved with it, flagged to Jeremy; **15% of net sales** now stated (192 anticipated a restaurant-confirmed figure); **mobile code RCFUND96** for app/online orders is in the description (an app order without it raises nothing); kiosk/catering/gift cards excluded. 🚨 **Cane's terms: the fundraiser "can not be advertised at the Restaurant during the event"**, so 193's "you can buy one there" merch promise was **removed from BOTH Cane's nights** (Oct 26 and Nov 9); "Wear your Mav shirt." stays. Followups merch-table item now covers Sep 15, Sep 30, Oct 14 only. **Not done:** `cover_image_url` not set to the flyer PNG (portrait, followups); Nov 9 gets no link/code — RCFUND96 is per-event, wait for Cane's Nov 9 page. Verified live: Oct 26 page reads 5:00-9:00, 15%, RCFUND96, button "Fundraiser Details →" to the Cane's URL, no "buy one there". `db/apply_all.sql`: 213/213. DB-only, no deploy.

**2026-09-22 (evening) — Three Oct/Nov spirit nights added (migration 210). Last migration applied: 210.** From Kendra's email to Jeremy 9/22: **Wed Oct 14 5–8 PM Pok-e-Jo's Smokehouse** (2121 Parmer Lane, 78727), **Mon Oct 26 5–8 PM Raising Cane's** (12901 N I-35 Bldg 21, 78753), **Mon Nov 9 5–8 PM Raising Cane's** (same store). Same shape and copy as 192/193/194 (published, venue row, "Please mention McNeil Football at the register" + the shirt/merch line carried verbatim and asserted). ⚠️ **5–8 PM, not 6–8** like September; verify block checks hour 17. Weekdays checked with `date`; no `events` or `games` row on any of the three dates. Two new `venues` rows, the Cane's row shared by both Cane's events. Addresses are Kendra's as written (no club file to cross-check; neither restaurant is in `coach-meals.ts`); `location` labels use only what her text supports ("Parmer", "N I-35"). Both restaurants are in `sponsors` (same tier). Slugs `spirit-night-pok-e-jos-2026-10-14`, `spirit-night-raising-canes-2026-10-26`, `spirit-night-raising-canes-2026-11-09` — 194's rule: never rename if a date moves. Verified live: all three `/events/<slug>` pages 200 with the right title, `/events` lists them, `/events.ics` carries 5 spirit-night VEVENTs. `db/apply_all.sql` regenerated: 212 bundled, 212 on disk. ⚠️ **Still open from 193: the copy promises merch on site, so three more nights need someone to bring the merch table** (followups.md). DB-only, no deploy.

**2026-09-22 (evening) — Varsity/JV: NO morning practice Wednesday Sep 23 (migration 209). Last migration applied: 209.** Jeremy, relaying Coach Tuesday evening: *"JV and Varsity... they do not have practice tomorrow morning. they will still be in flex and period. freshmen stays the same."* One day block in two rows: the 5:40 / 6:00 / 6:05–8:10 lines are gone and Wednesday now opens with the same **"No morning practice."** Thursday and Friday use, followed by a parenthetical that Coach changed it Tuesday evening and his published schedule had a 5:40 session (so a reader holding Coach's graphic knows the page is the newer fact). Period 2/6, the 10:45 flex line and the team dinner line kept verbatim; heading still says "JV game day". **Freshman body untouched**, and the verify block asserts its 8:00/8:25 Wednesday lines survive. This is a verbal relay overriding Coach's doc (120's rule normally goes the other way); taken because it cancels rather than moves a session and the failure mode is showing up at 5:40 to an empty field, not missing practice. 9/21 newsletter never mentioned the Wednesday morning session, so nothing else carries it. DB-only, no deploy. Verified live: `/schedule/practice/varsity` and `/jv` read "No morning practice" for Wednesday with Tuesday's 5:40 block intact; `/schedule/practice/freshman` (that is the route, not `/freshman/green`) unchanged at 8:00/8:25. `db/apply_all.sql` regenerated: 211 bundled, 211 on disk. Coach posted the change on SportsYou Tuesday afternoon himself, so families heard it before the page changed; nothing further needed.

**2026-09-22 — Wednesday kickoffs confirmed by Coach (migration 205); track venue pinned (206, 207) and renamed (208). Last migration applied: 208.** Coach, relayed by Jeremy: *"JV will play at 6:00PM at Lake Travis Track Stadium. The Freshman will play here at McNeil Stadium at 5:30PM."* Closes 203's two `tbd` rows in 130's shape (time + `scheduled` in one UPDATE, so they re-enter `/events` and the ICS at the right time). **Freshmen 5:30 at Maverick Stadium** — half an hour EARLIER than the school's 6:30, the direction that strands a family; the 9/21 newsletter said "do not assume 6:00" and pointed at the page, which is now right. **JV 6:00 at a NEW venue, `Lake Travis Track Stadium`**, a second field on the LTHS campus (155's Burger Annex pattern: second `venues` row, shared campus address). The game's `notes` reads "Track stadium, not Cavalier Stadium" so a family that went to Cavalier before does not go there again. Jeremy asked whether both Lake Travis fields already existed — **no**: before 205 the rows were `Cavalier Stadium` (pinned) and a campus-level `Lake Travis High School` (no pin), both now with zero games. 205 left the track pin NULL on purpose (unverified spot); **206 filled it from Jeremy's Google Maps place link** (30.3235418, −97.9704399, ~270 m SW of Cavalier), tracking query stripped. **207 moved the pin again** to Jeremy's second link, which resolves to Google's own listing **"Cavalier Track and Field Stadium"** (30.3233032, −97.9713134, ~90 m from the first) — the official listing is what a Maps search returns, so the link points there while the venue NAME stays Coach's "Lake Travis Track Stadium". The JV `notes` was reworded to "…not the Cavalier football stadium" so it does not contradict a map labelled Cavalier Track. **208 then renamed the venue itself to `Cavalier Track and Field Stadium`** on Jeremy's instruction (*"change the name in case they google it"*): venue name, the JV row's `location`, and the practice-body pointers ("at Cavalier Track and Field Stadium (Lake Travis)"). ⚠️ Two venues now begin "Cavalier"; the JV `notes` ("…not the Cavalier football stadium") is what tells them apart, keep it. **Both practice bodies had "to be announced" in their pointers (202) and 205 replaced that text in the same migration** (176 rule); verify block fails on any surviving "to be announced". Hidden Blue row left at 5:00 as always.
...
🚨 **GAME WEEK IS A DAY EARLY.** JV and freshmen play **Wednesday Sep 23** vs Lake Travis and varsity plays **Thursday Sep 24 at 7:00 p.m. at KRAC**; `games` already had all three on those days from the 057 seed. Both practice bodies open with a one-line ⚠️ pointer to the Games schedule (a pointer, not a copy of the game data — 181's line holds). The varsity team dinner is therefore **Wednesday** Sep 23, 6:00–7:30 p.m. on campus, from `lib/team-dinners.ts`, and is one line in the varsity/JV body — the club runs the dinner, so the club's slot is the source; this is the first week the practice page and the Team Dinners page agree on it instead of "time to be announced".

**203 — JV (6:00, Lake Travis HS) and freshman Green (6:30, Maverick Stadium) on Sep 23 set to `result_status = 'tbd'`.** Coach's doc says "Time TBA – Location TBA" for both and Jeremy said the times are TBD. This is deliberately NOT what 181 did in Week 6 (leave the school's times and hedge in the body): Jeremy has now said outright the times are unknown, and `tbd` is the status built for that (078/130) — the games table and cards print **TBD** in the time cell, and **`game-events.ts` excludes tbd rows from `/events`, the month view and the ICS on purpose** so a placeholder time never publishes as real. ⚠️ **Cost: both games are off the calendar and the ICS until Coach gives times.** When he does, the fix is 130's shape — UPDATE `game_date` AND set `result_status` back to `'scheduled'`, one migration, per row. Locations were NOT touched (Jeremy said times only; a venue moves only on Coach's graphic, 155) and the hidden Blue row stays `scheduled` at 5:00 (155/173/191/199 call). Verified live: `/schedule/games/jv` and `/schedule/games/freshman/green` read **TBD** for Lake Travis with no 6:00 / 6:30 anywhere; varsity Sep 24 unchanged; all three practice pages read Week 8 with no Labor Day or Week 6 text. DB-only, no deploy.

**2026-09-18 — Varsity roster r5: four in, one out, Hicks to a single number (migration 201). Last migration applied: 201.** Jeremy: *"varsity roster updated. please update the website and the downloadable pdf."* The coaches' new export (`McNeil Varsity Roster - Var Roster 8_21.pdf`, a Google Sheets PDF of the same sheet 157 was seeded from) was parsed by value and diffed against the live 46-row roster by jersey AND by name, per 168/200. **Seven changes:** #15 Lucas Rosilmo removed; **#15 Antonio Showels RB So.**, **#24 TK Keller LB So.**, **#37 Kevonn Conner DB/WR Sr.**, **#82 Henion Kim K Fr.** added; **Jace Hicks `64/65` → `64`**; the Trainers credit drops Xander Keller (PDF only). Roster is 46 → 49, sort_order re-densified 1..49 by leading number. Rosilmo is DELETEd, not deactivated, because every varsity guard since 183 counts rows not active rows; his row is in `201_rollback`. **Showels is also JV #0 and Kim is also freshman #10 — playing up, 183's rule, do NOT deduplicate.** Kim's grade is `Fr.`, the form every 2025-26 freshman row uses.

⚠️ **HICKS `64/65` → `64` IS THE OPPOSITE OF 171'S WARNING AND WAS STILL APPLIED.** 171 says slashes are real home/away pairs and must not be tidied; this is not tidying, it is the coaches' own export listing him as 64 only, and the page and poster follow the coaches' artefact (166/168/186/200). Three slash jerseys remain and 201 asserts exactly three. Flagged to Jeremy.

⚠️ **ONE DELIBERATE DIVERGENCE FROM THE COACHES' PDF: #10 stays `Aymane El Anssari`.** The new export spells him `Elanssari`; the 2025 JV PDF (031) and the original 2026 varsity workbook (157) both say `El Anssari`, so the staff's artefacts disagree 2:1. Kept the established spelling on BOTH the page and the r5 PDF so they agree with each other; the workbook cell was left alone and `update_workbook.py`-style rewrites must keep that override. Flagged to Jeremy; if `Elanssari` is right it is one UPDATE + a workbook edit + an r6 upload, never a DB-only fix (176 rule).

**The PDF half, same sitting (178/186).** The coaches' workbook (`roster_pdf/source/Varsity McNeil Roster 2026.xlsx`, gitignored) was rewritten to the export's two-block layout — now **24 left / 25 right**, two data rows inserted, staff block merged range moved from A27:H31 to A29:H33 — and `scripts/make-varsity-roster-pdf.py` regenerated it: one page, `49 players: 24 left block, 25 right block`, diffed by value against the coaches' PDF with only the El Anssari divergence. ⚠️ pdfplumber wraps the coaches' Assistant Coaches line at "Nicolas / Edwards"; that break was re-joined before writing the staff cell, or the generator prints "Nicolas" as its own credit line. Uploaded as **`documents/rosters/varsity-2026-r5.pdf`** (new filename per 158, `apikey` header per 186, 120,219 bytes, sha256-identical round trip); r4 left in the bucket for the rollback. Coaches' PDF pinned at `roster_pdf/source/varsity-2026-r5-source.pdf`; the r4 workbook and PDF archived under `roster_pdf/source/archive/`. **Verified live:** `/roster/varsity` reads `15 Antonio Showels RB So.`, `24 TK Keller LB So.`, `37 Kevonn Conner DB/WR Sr.`, `82 Henion Kim K Fr.`, `64 Jace Hicks OL So.`, no Rosilmo, no `64/65`, Print View on r5. DB-only, no deploy.

**2026-09-17 (later) — Freshman roster r2: #19 Khalil Miller plus three changes Jeremy did not see (migration 200). Last migration applied: 200.** Jeremy: *"please add #19 Khalil Miller to the freshman roster. and replace the downloadable pdf...at a glance, that was the one change I saw."* The coaches' revised PDF (50 players, now one page) was diffed programmatically against the live `freshman-2026.pdf` (49, two pages) by jersey AND by name, per 168's parse-by-value rule. **Four changes, not one:** #19 Khalil Miller added; **Quinten Spurlock #30 → #35**; **Adam Zayad #74 → #56 and Andy Fernandez #56 → #74** (a clean swap). All four applied so the page and the poster agree with the coaches' artefact (166/168/186 rule). Roster is 50, sort_order re-densified 1..50 in jersey order. PDF uploaded as **`documents/rosters/freshman-2026-r2.pdf`** (new filename per 158, `apikey` header, sha256-identical round trip; original left in the bucket for `200_rollback`). Verified live: `/roster/freshman/green` reads `19 Khalil Miller`, `35 Quinten Spurlock`, `56 Adam Zayad`, `74 Andy Fernandez` on desktop and mobile, Print View on r2. Source PDF pinned at `MavericksWebsite/roster_pdf/source/freshman-2026-r2-source.pdf` (gitignored).

⚠️ **THE ZAYAD / FERNANDEZ SWAP IS EXACTLY WHAT A SPREADSHEET SLIP LOOKS LIKE, AND ALSO EXACTLY WHAT A REAL REASSIGNMENT LOOKS LIKE.** Applied because the coaches' PDF is the source of record and 186 refuses a PDF the DB disagrees with; flagged to Jeremy in the same message. If the coaches say it was a mistake, the fix is a corrected PDF plus a reverse-swap migration, never a quiet DB edit that leaves the poster wrong.

🚫 **"AT A GLANCE" IS NOT A DIFF.** A one-change request against a re-exported document should always be diffed against the live artefact before editing; the other three would have shipped a poster and a page that disagreed with each other on three numbers.
...
🚨 **THE INCLUSIVE SENTENCE IS THE REASON THIS EVENT'S COPY EXISTS, AND THE MIGRATION ASSERTS BOTH HALVES.** Jeremy asked for it by name: *"I'd like to include some text that's a bit sensitive for boys who don't have a solid mom figure."* The line is **"Bring your mom, stepmom, grandma, aunt, or whoever has been your person. We want them there with you."** 🚫 Do not trim it to "bring your mom", and do not drop the second sentence, which is what turns a list of options into an invitation. ⚠️ The attire line reads **"She wears"**, never "Moms wear" — re-narrowing to mothers two sentences after widening would undo the whole point, and there is a guard against that exact string. The event NAME keeps "Mavs and Moms" because that is what the tradition is called; the copy does the work, not a rename.

⚠️ **FIRST `events` ROW WITH A NULL `ends_at`, AND IT IS A SUPPORTED STATE, NOT AN OVERSIGHT.** No end time was supplied. `formatEventWhen` renders "September 20 @ 12:30 PM" with no range and `lib/queries/events.ts` falls back to end-of-day for upcoming/past filtering, a fallback its own comment calls load-bearing. **Inventing a finish time would put an unsourced fact in front of a family** — same rule that left the freshman/JV pickup time null for weeks. ⚠️ Note the ICS route emits `DTEND` one hour after `DTSTART` as its own default; that is a calendar-format necessity, not a published claim, and it appears in subscribed calendars only.

🚨 **`cover_image_url` IS STILL NULL ON EVERY ROW, AND THE FLYER IS WHY, NOT AN OVERSIGHT.** Jeremy supplied artwork and the column exists for it, but **both renderers force `aspect-video` + `object-cover`** and the flyer is **820x1024 portrait**. Covering 16:9 from a 0.8 ratio keeps the middle ~45%: the title crops off the top, the entire WHO / WHAT / WHEN / WHERE / ATTIRE block crops off the bottom. This would have been the column's first use and its first bug. **Every fact on the flyer is in the description text regardless** — the standing never-only-in-an-image rule, load-bearing twice here. Fix is a landscape flyer or a renderer that tolerates portrait; see `followups.md`, which also notes this quietly invalidates the "restaurant artwork is a data change, no code" assumption in the spirit-night flyer item.

⚠️ **IT COLLIDES WITH THE COACHES SUNDAY LUNCH DROP-OFF**, 12:30 to 1:00 the same day at the same building (Sep 20, Mighty Fine, already claimed). Not something the site can resolve and not a reason to move either, but if that volunteer is also a senior mom, somebody needs to know. Flagged.

**2026-09-14 — Varsity Week 6 result, and the broadcast links finally came down (migration 196). Last migration applied at the time: 196.** Jeremy: *"varsity lost 55-3 last week."* Stored `our_score = 3, their_score = 55` (ours first, per 170/174/182), rendering **L 3-55** on `/schedule/games/varsity`.

🚨 **MARKING A GAME FINAL IS THE ONLY THING THAT TAKES BROADCAST LINKS DOWN, AND THAT DEPENDENCY COST SOMETHING FOR THE FIRST TIME.** 195 inserted the Rouse VYPE and YouTube rows with `keep_after_final = false`, exactly as 180 requires. But `keep_after_final` is evaluated against `result_status`, so with no score entered **both links stayed published for three days on a game that was over** — under a policy written precisely because Bowie's coach asked for a replay to be pulled. 174 and 180 each flagged the dependency in the abstract; this is the instance. 🚫 **If links ever need to come down and the score is not to hand, deactivate the rows explicitly. Do not wait on the result.**

⚠️ **ONLY VARSITY WAS SUPPLIED, AND THE BACKLOG IS NOW TWO WEEKS DEEP.** The Thursday **Sep 10** JV and both freshman rows are still `scheduled`, on top of the **Sep 3** rows 182 left open for the same reason. Guessing is worse than `scheduled`, so the post-hoc guard stays scoped to varsity and excludes scrimmages. **Four sub-varsity results are outstanding.**

**2026-09-13 — Freshman/JV meal pickup moved to 2:15 p.m., drop-off 2:45. No migration, and that is the point.** Coach wanted the food at the school on time after traffic delayed a run; Lareina told Bush's. Jeremy changed the Google Form and the live Apps Script himself and asked for the website half. **These are CODE CONSTANTS in `lib/fresh-jv-meals.ts`, so unlike a DB edit they needed a commit and a deploy** (`944404a`). Verified live on `/boosters/fresh-jv-meals`; the page, the slot table and the FAQ all read the same two constants.

🚨 **THE APPS SCRIPT CARRIES A THIRD TIME VALUE AND CHANGING ONLY TWO WEAPONISES ITS OWN GUARD.** `dailyFreshJvMealReminders` compares `PICKUP_TIME` against `SEASON_PICKUP_TIME` every morning and mails Jeremy when they differ — **and the mail tells him to set PICKUP_TIME back.** Change one without the other and the 9am job spends every day campaigning for the retired time. All three moved together in the mavericks-ops copy, plus the `DROPOFF_TIME` that was hardcoded in the nag body. Jeremy pasted it into the live project the same day.

🚨 **THIS IS THE SEASON'S THIRD PICKUP TIME (2:00, 2:30, 2:15) AND THE STANDING RULE HAS FIRED.** Both `lib/fresh-jv-meals.ts` and `fresh-jv-meals-automation.gs` say in capitals that two values is a global plus a reminder and three means **the time belongs on the slot**, copying `team-dinners-automation.gs`. **Deliberately not done** — it spans four files and the next pickup was two days out. The global still states the truth only because 2:15 applies to every remaining night. See `followups.md`.

⚠️ **SENT CONFIRMATION MAIL STILL SAYS 2:30 AND NOTHING REWRITES IT.** Reminders re-read the constant and will say 2:15, so volunteers are corrected if they read them. The 9/14 newsletter says so explicitly, which is the only channel that reaches all seven at once.
...
🚨 **P2/P6 MOVED TO 10:45, HALF AN HOUR EARLIER, EVERY DAY IT RUNS** (was 11:15), and Coach added a **FLEX OUT** line to match: "be on the field by 10:45." ⚠️ **Flex out is NOT glossed.** P2/P6 gets a gloss because we know what it is; nobody here has confirmed what "flex out" releases them from, and a guessed gloss on a schedule is worse than Coach's own shorthand.

🚨 **"Lunch after — all C lunch" WAS DROPPED, ON PURPOSE.** It rode on Mon/Tue/Wed in Weeks 4 and 5 and is not on this week's doc. It is not carried forward: the block it hung off just moved thirty minutes earlier, which is exactly the circumstance where a stale lunch assignment would be wrong. **Absence of a line is not evidence the fact still holds.** If C lunch is permanent, Coach saying so puts it back. The migration guards that it is gone.

Sept 7 is now a real day *inside* the week, so Labor Day moved from the "After Week 5" block to a day heading — the top 🚨 callout stays anyway, per 172's reasoning that a school holiday reads as "no practice" by default.

**182 — varsity Game 2 result: LOST 24-30 to Lake Belton at KRAC.** Jeremy: *"lost heartbreaker last week 30-24 (was a great hard fought game)."* Ours-first as always, so `L 24-30`. ✅ **180's short-lived broadcast design worked with no data change**: both Sep 4 rows carry `keep_after_final = false`, and flipping the game to `final` dropped the YouTube and VYPE links off the row. Verified live.

⚠️ **THE GUARD FIRED AND IT WAS RIGHT.** A first draft of "no past varsity game may read scheduled" failed on the **Aug 13 Hendrickson and Aug 20 Eastview varsity scrimmages** — still `scheduled` weeks later, exactly the eight rows (four levels × two dates) 174 documented and declined to fix. 174 excluded them by date; 182 excludes them with **`notes <> 'Scrimmage'`**, which is the clean selector 176 found and which keeps working all season where a date scope silently stops protecting anything. 🚫 **Excluding them is not resolving them** — all eight still show in `/events`, the month view and the ICS as upcoming.

🚨 **THE SEP 3 GAMES ARE STILL `scheduled` AND STILL NEED RESULTS** — JV vs Lake Belton and **both** freshman rows, Green and the hidden Blue (170's rule). No result was supplied and guessing is worse than the wrong state. This is the third straight week a non-varsity result has lagged.

**183 + 184 — #67 Aiden Ross, OL, junior, added to varsity (45 → 46).** ⚠️ **HE IS ALSO ON JV AND THAT IS NOT A DUPLICATE.** `/roster/jv` carries him as **#66 Aiden Robert Ross**; Jeremy said explicitly that playing up is fine. 🚫 Do not "deduplicate" — deleting the JV row drops him off the JV page. **Three things differ between the two rows on purpose:** jersey (varsity 66 is already Kaeden Frazier, so 67 cannot be a typo for 66); name (the coaches' workbook lists full legal names with middles on the JV sheet and none on the varsity sheet — short form belongs here, and it is how 2025-26 listed him); and position, which 183 left null because Jeremy supplied only number, name and grade. **184 filled OL after asking him** — sourced from that same workbook's JV sheet, where class 11 matches the junior he gave. 🚫 **Not a precedent for backfilling the other empty Position and Grade cells**; those stay em-dashes (Jeremy 2026-08-26).

`sort_order` was **recomputed, not hand-patched** — 159's convention as 171 applied it, dense from 1 by the leading number of `jersey_number`. The migration first asserts the recomputation is a no-op for the existing 45, so a silent reshuffle of the whole roster cannot hide inside the insert. Seven rows differ: the insert plus the six above it.

**186 — the print roster is r4, done in the same sitting.** The workbook (`roster_pdf/source/…xlsx`, gitignored, only on Jeremy's disk) gained one row in the RIGHT block between #66 and #71; E19:H24 shifted down, columns A–D untouched. **Blocks are now 23/23** — 171 left them 23/22 and 46 players simply balances; 159 and 166 both say do not reflow the print layout into one column. New filename per 158's cache rule (Storage serves `no-cache`, so Next falls back to a 31-day TTL and an overwritten object can serve stale bytes for a month); r3 stays in the bucket for the rollback. Upload needs the key in the **`apikey` header** as well as `Authorization`. Verified: one page, "46 players: 23 left, 23 right", extracts `67 Aiden Ross OL 11` between Frazier and Omagbon, staff block intact, and the served object is sha256-identical to the local file. **186 refuses to ship a PDF claiming 46 players unless the DB agrees** — 178's pattern.

**185 — Game Photos now points at this season.** The row led to a Google Doc titled *"2025-2026 McNeil Football Photos"*, owned by **djohnsonjr@gmail.com** (a parent, not the club), last modified 2026-01-20, whose newest entry is the January banquet. Nothing from 2026-27 was in it and nobody here can add to it. Jeremy chose "replace previous link", so it now goes to the **Bowie album** (`20260828 MHS Varsity Football at Bowie`, verified 200 + og:title before writing). The retired doc URL is preserved in the migration and its rollback.

⚠️ **THIS TURNED A DURABLE ROW INTO A WEEKLY TASK.** 114 gave `/resources` exactly ONE Game Photos row and rejected a row per album. Still one row — but its destination is now a single game, so it goes stale the moment the next album exists. **It joins the VYPE broadcast links as a standing week-by-week update.** The permanent fix is a 2026-27 index doc the row can point at again; that needs djohnsonjr. Two warnings carry forward unchanged from 114/131: a Google Photos share link is **public to anyone holding it and these are photos of minors** (Jeremy's standing call), and **album links rot silently**.

**Verified on prod** after all six: varsity/JV and freshman practice pages read Week 6 with the graphic's Labor Day times and no trace of Week 5, 11:15, "C lunch" or 175's wording; `/schedule/games/varsity` reads `L 24 - 30` with both broadcast links gone; `/roster/varsity` reads `67 Aiden Ross OL Jr.` between Frazier and Omagbon on **both** the desktop table and the mobile card, Print View on r4; `/resources` Game Photos points at the Bowie album with no trace of the old doc.

## Earlier entries (updated 2026-09-01)

**2026-09-01 (evening) — post-meeting batch: rosters lose height/weight, Whataburger to Platinum, broadcast links become short-lived (migrations 179, 180 + a deploy). Last migration applied: 180.**

🚨 **HEIGHT AND WEIGHT ARE GONE FROM EVERY ROSTER SURFACE, AND THIS IS NOT THE "EMPTY COLUMNS" RULE.** Coach, relayed by Jeremy: *"this coach doesn't like to announce that info to other teams."* Competitive call, not a data-quality one. **Position and Grade still render with em-dashes when empty** — that is the OPPOSITE rule (Jeremy 2026-08-26: *"maybe it will make the coaches want to step up their game on getting me data!"*) and it stands. Two rules pointing opposite ways now live in `components/roster/player-table.tsx`, documented at the top of that file, and `followups.md` was rewritten so nobody merges them. ⚠️ **The mobile card was a SECOND code path** — `mobileSep([height, weightText])` would have kept publishing both on every phone after the desktop columns were gone. The DB columns and the `Player` type are untouched, so Coach can reverse this with no data loss.

**Varsity Team Dinner copy: the host provides EVERYTHING.** Plates, bowls, plasticware, napkins, serving utensils and drinks, not just food. Nothing is stocked at the school. Plus a matchmaking address: `teammeals@mcneilmavericks.org` connects families who want to team up but have nobody to team up with. ⚠️ **That is a SECOND deliberate reversal** — a 2026-08-26 comment said *"Do not restore the matchmaking promise"* because the old copy promised connecting people with no owner. It is different now because a real address does the matching; self-organising stays primary, the address is the fallback. The reversal is recorded in `lib/team-dinners.ts` and the page.

⚠️ **THE APPS SCRIPT AND THE LIVE GOOGLE FORM STILL SAY THE OLD THING** — `dinnerBlock()` in `team-dinners-automation.gs` (every confirmation and reminder), its teaming paragraph, and the form description. Exact replacement text is in the 2026-09-01 handoff; the form is a hand-edit because the generator only runs at creation. **Fourth instance of the artefact-that-no-migration-reaches problem.**

**179 — Whataburger Gold → Platinum.** Platinum is CLOSED for sale (`available = false`) and this must not reopen it; adding or promoting at a closed level is normal (164 did it for Gold) because the roster surfaces filter tier `active`, not `available`. 179 asserts Platinum is still closed when it finishes. `sort_order` recomputed from `tier price_cents DESC, name` per 154/164/167, never hand-shifted: Platinum 1-4 (Airborne, Capstone, North Austin Oral Surgery, Whataburger), Gold 5-12, Blue 13-17. Platinum draws logos larger (200x72 vs 150x58), which makes 169's full-colour logo fix more visible, not less.

🚨 **180 — A BROADCAST LINK IS NO LONGER AN ARCHIVE.** 165 set `keep_after_final = true` on YouTube rows on the theory that they persist as a replay. **That theory is retired.** Merle Bertrand at VYPE: *"Coach never responded about hiding the replays, but I had to hide last week's at the request of Bowie's coach."* Jeremy: the links should *"only be good for about 24 hours after the game."* So the Aug 28 rows are deactivated (not deleted) with `keep_after_final` cleared, and the Sep 4 rows went in with `keep_after_final = false`. **Do not set it true again on the strength of 165's comment** — opposing coaches ask for film to come down and the club is not the party that gets to refuse. ⚠️ `keep_after_final` only fires when a game is marked `final`, so this is an approximation of "24 hours" that **depends on somebody entering the result promptly** — the dependency 174 flagged is now load-bearing for something a coach has actually complained about.

Week 5 links verified before insert: both HTTP 200 under a desktop UA and both titles read "7PM - Football: McNeil vs. Lake Belton", naming this week's opponent. That title check is the standing procedure and it is what catches a link pasted from the wrong week.

⚠️ **A `Watch` row on a 2025-11-07 game vs Hutto points at `youtube.com/@iHSFan`, a channel homepage rather than a game.** It is LAST season's row, which is why 180's guards scope to `year = '2026-27'` — a first draft excepted it by date, assumed 2026, and failed. Left alone.

**2026-09-01 — the varsity home opener moved stadiums, three days out (migrations 177 + 178). Last migration applied: 178.** Fri Sep 4 vs Lake Belton: **Round Rock High School Dragon Stadium → Kelly Reeves Athletic Complex**, `location` 'Dragon Stadium' → 'KRAC'. Jeremy. **Kickoff does NOT change** — he restated 7:00 p.m. and the row already held 19:00, so 177 deliberately does not touch `game_date`; confirmed before writing rather than assumed.

**'KRAC', not the full name**, because every other Kelly Reeves game in `games` writes exactly that (verified: it is the only distinct value) and the school's PDF uses KRAC in its SITE column. Matching the surrounding rows wins — the same call 155 made for 'Maverick Stadium'. The `venues` row carries the full name and the verified pin, and it already existed, so this is a repoint rather than a second Kelly Reeves row with a guessed address.

✅ **177 AND 178 WERE APPLIED IN THE SAME SITTING, WHICH IS THE WHOLE POINT.** The day before, 176 had to clean up twelve days of the site and the printed schedule disagreeing because 151 changed the data and nothing changed the artefact. Splitting a game-data change from its PDF patch is that bug; here the PDF was regenerated from the school's original (19 audited edits, still one generation deep), uploaded as `2026-27-r3.pdf`, and all four roster rows repointed before the session ended. **178 also guards that `games` actually says Kelly Reeves before it will ship a PDF claiming so.**

⚠️ **The 8/31 newsletter named Dragon Stadium and 201 Deep Wood Drive twice**, in a paragraph whose entire point is "put the address in your phone now". `make_newsletter.py` now carries `VENUE` = Kelly Reeves, 10211 W Parmer Lane, and the file was rebuilt. **If that issue already went to parents, this needs a short correction email, not a silent regeneration** — the wrong stadium is the one error in that email that actually strands a family on a Friday night.

Verified on prod: `/schedule/games/varsity` reads KRAC with Print View on r3 and no "Dragon Stadium" anywhere, `/events` shows Varsity vs Lake Belton at KRAC, and the ICS feed carries `LOCATION:KRAC, 10211 W Parmer Lane, Austin, TX 78717`.
...
**173 — kickoff times, split from 172 deliberately** (a time is what a family plans an evening around, and it should roll back on its own). JV Sep 3 **6:00 → 7:00**; freshman Green Sep 3 **6:30 → 5:00**. The second is 155's Aug 27 change repeated, exactly as 155 predicted it would need to be: the school's footnote "Blue @ 5:00 / Green @ 6:30" is a TWO-TEAM stagger and McNeil fields one freshman team (148), so 6:30 is the stale half of a pair. Varsity Sep 4 already matched at 7:00 and is untouched — recorded so its absence reads as a finding, not an oversight.

⚠️ **The remaining eight Green rows still say 6:30 and eight JV rows still say 6:00.** Two straight weeks have now come in at freshman 5:00, so it looks like the season shape, but **Jeremy's call 2026-08-31 was to keep it week-by-week** per 155's rule. Do not bulk-flip without Coach confirming.

**174 — varsity opener result: LOST 14-62 at Bowie.** Ours-first, same as 170. ✅ **This finally exercised the post-final broadcast render that had been an open followup since 165**: YouTube persists (`keep_after_final = true`, it is the replay), VYPE drops off (`false`). Verified live. 🚨 **It also surfaced that eight played scrimmages still read `scheduled`** (Aug 13 Hendrickson, Aug 20 Eastview, all four rows each) and therefore still appear in `/events` and the ICS feed as upcoming. An unscoped "no past game may be scheduled" guard failed on them and had to be narrowed to Aug 24 onward. **Unresolved, needs a convention** — see followups.

⚠️ **A RESULT CAN SIT WRONG FOR DAYS AND NOTHING COMPLAINS.** Friday's score was still unentered on Monday. `scheduled` on a past game is the worst state: it reads as a game still to come. **Writing the weekly newsletter is the checkpoint** — it needs the score anyway, so query for past games still `scheduled` before drafting.

**Newsletter (`~/Projects/BoosterClub/`, outside this repo).** `make_newsletter.py` content block rewritten for the 8/31 issue; 18 paying sponsors scraped live (was 17). Sep 3 and Sep 4 are IN this issue, which the 8/24 issue deliberately held back while the freshman kickoff was in doubt — 173 resolved it. ⚠️ **The 8/24 issue shipped the horseshoe drop-off for coach meals, which was already wrong when it went out**; this issue says inside the Player Drop-off Doors, and `draft_newsletter.mjs` now hard-aborts on the old wording. New `draft_newsletter.mjs` creates the HTML draft in Jeremy's mailbox reusing the JV Assistant Gmail token; **draft-only by design, no send flag** — do not add one.
...
✅ **`PICKUP_TIME` FLIPPED TO `2:30 p.m.` / `3:00 p.m.` ON 2026-08-29**, week one (Thu 27 Aug) being past. Week one was 2:00; **every remaining night this season is 2:30**, so do not put 2:00 back — it belongs to one night that has already happened. Changed in three files: `PICKUP_TIME`/`DROPOFF_TIME` in `fresh-jv-meals-automation.gs`, `FRESH_JV_MEAL_PICKUP_TIME`/`_DROPOFF_TIME` in `lib/fresh-jv-meals.ts`, and the `create-fresh-jv-meals-form.gs` generator (which was never updated when the time was confirmed and would have regenerated "still confirming" plus the horseshoe). ⚠️ **Editing the repo changed nothing that runs**: the .gs must be pasted into Apps Script (check `DRY_RUN = false` survives) and the site must be redeployed. The daily nag's guard is now satisfied and silent, and it never could see the repo anyway — the email stopping is still not proof the site shipped.

🚨 **THE LIVE FRESHMAN/JV GOOGLE FORM STILL SAYS HORSESHOE.** Fetched and parsed 2026-08-29. Its description and the date question's help text both still carry *"We are still confirming the exact pickup time - plan on mid afternoon"* **and** *"Bring it to the Horseshoe lot at McNeil. Coach Hale will meet you there."* When the horseshoe was corrected in four places on 2026-08-26, and when the coaches' form was verified clean the same day, **this form was not in the set** — the generator runs only at creation, so nothing propagates to it and nothing watches it. It has been handing the exact wrong instruction to every volunteer since. Paste-ready text: `docs/fresh-jv-form-hand-edit-2026-08-29.md`. **The lesson generalises: a live Google Form is a fifth surface, and it is the one no code path can reach.** Any time a time, place or contact changes, count the forms too.

⚠️ **Nine claimed nights were confirmed at 2:00 and their confirmation emails cannot be recalled.** Seven volunteers hold all nine remaining nights (Sep 3 → Oct 29). Reminders re-read `PICKUP_TIME` at send time and will say 2:30; the confirmation already in the inbox will not. Whoever flips a time owes those people a note.
...
⚠️ **WHATABURGER'S SUPPLIED LOGO WAS 77x75 AND ENTIRELY WHITE, AND SHIPPING IT WOULD HAVE BEEN A BLANK CARD.** It is whataburger.com's own nav asset (`assets/images/navigation/Whataburger-logo.png`) — the *reversed* variant, meant for use on their orange. The sponsor cards are white. **This is the limit of the Batrice "publish the best available asset anyway" rule** (Jeremy 2026-08-22): that rule exists *because* a null `logo_url` renders as nothing, so honouring it with a logo that also renders as nothing defeats its whole purpose. The mark used instead is Whataburger's own current lockup in vector and in their orange — Wikimedia Commons `Whataburger logo.svg`, public domain — confirmed current (not a dated mark) by rendering their site's own white wordmark SVG over orange and matching the hand-drawn lettering glyph-for-glyph, with their orange app icon confirming the orange/white pairing. **Standing rule this sets: composite a supplied logo over WHITE before accepting it.** Swap the file if Whataburger sends real artwork.

⚠️ **The Commons SVG has `width`/`height` and NO `viewBox`, which is a silent-blur trap.** Without a viewBox a CSS width just enlarges the canvas and leaves the art at its authored 300px, so a crop-then-scale pass **upscales 301px by 4x** and looks like it worked. `prep_gold_2026_08.py` synthesizes the viewBox from width/height and `crop_and_scale` now **aborts rather than upscaling**. Watch for this on any SVG that isn't ours.

**Cane's sent a brand style guide, and two of its rules bind on the PHYSICAL Gold deliverables, not the web card:** never below **1 inch wide**, and **1/4x clear space** on every side. The stored PNG is cropped tight to the ink on purpose because the card supplies its own padding — so **the field signs and the McNeil Drive sign must be laid out from the vector PDF**, not this PNG, with the clear space added back. Their improper-usage list also forbids recolouring, rotating, uneven scaling, recreating the logo, and removing the ® (the prep script asserts the ® survives the crop).

**Prep tooling and pinned sources: `MavericksWebsite/partner_logos/`** (outside the repo, same convention as `sponsorship_letter/` and `coach_photos/`). `sources/gold-2026-08/` holds the Cane's full-colour PDF + their style guide, the Pok-E-Jo's vector, the Commons Whataburger SVG, and `whataburger-REJECTED-white-77px.png` kept only to record what was rejected and why. `prep_gold_2026_08.py` rebuilds all three outputs at 1200px from those sources — **never from ~/Downloads**, the rule `prep_logos.py` documents. All three are vector-sourced with **real white fills** (the Cane's script, the Pok-E-Jo's sign field and SMOKEHOUSE knockout), asserted by compositing over two very different grounds; there is no white-keying anywhere, which would have eaten the Cane's wordmark outright.

**`sort_order` was RECOMPUTED, not appended** — 154's pattern, copied. Under this DB's collation the three land at Gold 6 (Pok-E-Jo's), Gold 7 (Raising Cane's) and Gold **12** — Whataburger sorts *after* `W Homes Collective`, because the space in "W Homes" is **not** ignored. All three positions are pinned in the migration's assertion block so a collation change fails loudly instead of quietly reshuffling the page.

**No deploy needed, and the newsletter needs no edit either** — `make_newsletter.py` scrapes `/sponsors` at build time and takes any heading ending in `" Sponsors"`, so the next issue picks all three up on its own.

**2026-08-24 — Varsity Helmet Decal Night (migration 156). Last migration applied at the time: 156.** Coach's flyer: parents and guardians put the decals on the varsity helmets, **Thu Aug 27, 5:00-6:00 p.m., McNeil football locker room**, the night before the season opener. Published and verified on `/events` and `/events/varsity-helmet-decal-night-2026`.
...
✅ **RESOLVED 2026-08-25 — THERE IS NO SEASON-WIDE FRESHMAN SLOT, so nothing is "90 minutes late".** This entry previously said the Aug 27 graphic had answered "which kickoff slot does the single freshman team play" and that the other nine games were very likely 90 minutes late. **That premise is retired.** Jeremy, 2026-08-25: the freshman kickoff *"could shift week to week because they don't have a green and blue team. 1 team means they could play early or late depending on the school opponent."* The old Blue-5:00 / Green-6:30 pair were two alternative slots for two teams that no longer exist, so neither is a default and there is no pattern to infer. **Aug 27 at 5:00 is correct and stays** (migration 155). The other nine are not wrong-by-90-minutes; they are simply unverified. 🚫 **The bulk `UPDATE` that used to be quoted in `followups.md` has been DELETED — do not reconstruct it**, and do not collapse the freshman `5:00/6:30` column in the Print View PDF either. **Take each week's freshman time from Coach's weekly graphic and change that one row**, the way Aug 27 was done. Sep 3 is the next one; wait for the graphic.
...
**2026-08-19 — homepage Upcoming Events now includes VARSITY games (`c8ca782`).** The strip read the `events` table only, so the whole season schedule was invisible on the homepage and it looked stuck at one card. It wasn't truncating: the limit was 2 and there is only one upcoming `events` row right now (Picture Day). Now `getUpcomingEvents(3, { includeGames: true, gameLevels: ["varsity"] })`. See rule 2 in the events-query section for why **varsity-only** is load-bearing and must not be widened to a bare `includeGames: true`. `game-events.ts` already documented `levels` as "the homepage uses it to take varsity" — the intent was designed and written down, the call site just never passed it. Worth checking for other options documented as if wired up.

**2026-08-19 — coaches meal pickup is 12:30, DROP-OFF is 1:00 in the Horseshoe lot** (`d215e59`, `b139b33`). "Between 12:30 and 1:00 p.m." read as one loose window; they are two separate commitments and volunteers were treating 1:00 as a soft edge. `COACH_MEAL_PICKUP_TIME` / `DROPOFF_TIME` / `DROPOFF_PLACE` in `lib/coach-meals.ts` mirror the same-named constants in `MavericksWebsite/scripts/coach-meals-automation.gs`, which is what the confirmation and reminder emails say — **site and email must not disagree about where to be and when.** The combined `COACH_MEAL_WINDOW` label is **deleted**, including from the per-date rows, which now stack "Pick up 12:30 p.m." / "Drop off 1:00 p.m.". Do not reintroduce a combined range.

✅ **The live Google Form is FIXED — verified 2026-08-26** (cache-busted fetch of the published viewform). Description reads "pick it up at 12:30 and drop it off at 1:00 p.m."; the date question's help text reads "Pickup is 12:30 to try and drop off at McNeil around 1:00 p.m." The old combined "between 12:30 and 1:00 p.m." is gone. **It had been fixed for some time while this file still listed it as the one unfixed volunteer-facing surface** — the note was stale, not the form. The underlying rule still governs any new copy: **12:30 and 1:00 are two separate commitments, never a window.**

⚠️ Hand-editing that form remains constrained to the **description and help text**. **Never** touch the checkbox option strings (matched byte-for-byte by the website prefill links and `parseDates()`) or add / delete / reorder any question (the automation reads the response sheet by COLUMN POSITION, not header name).

**Rudy's manager contact is now Brian Hickson alone** — Val Martin removed at Jeremy's request (2026-08-19), in `coach-meals-automation.gs` and `followups.md`.

⚠️ **SportsEngine is CANCELLED and is not an available tool.** Confirmed in the 2026-07-23 entry below. `MavericksWebsite/credentials.md` and `sportsengine_capture.md` still read like a live $1,299/yr plan and led to it being recommended in conversation this session; both now carry a CANCELLED banner. Don't propose SE for messaging, hosting, or anything else.

⚠️ **`MavericksWebsite/` is NOT under version control** — only `MavericksWebsite/BoosterClubWebsite/mavericks-website/` is a git repo. The eight Apps Script generators, `coach-meals-automation.gs`, `credentials.md`, and the specs above exist **only on Jeremy's disk**, with no history and no backup. Edits to them cannot be committed. Worth putting under git.
...
**Migrations 149 + 150 applied. Last migration applied: 150.** Commit `cd8bb29`, pushed as `jeremyvest-ATXcoder`. `tsc` + `next build` clean; the repo's only two eslint errors (`HeroCarousel`, `resource-item`) are pre-existing and untouched. Verified on prod: the button renders `ORDER PHOTOS →` on the vendor URL, **pool party still renders `Sign Up →`** (the regression that mattered), and all three practice pages render the anchor.

⚠️ **TWO CLAUDE SESSIONS WERE EDITING THIS REPO AT THE SAME TIME, and a `git status` snapshot cannot tell you that.** Three modified coach-meals files were sitting in the tree when this work started. They were NOT abandoned or stale — another session was mid-commit, and it went on to ship `d215e59`, `b139b33`, `2ac653a` and `c8ca782` (migrations 151 + 152) on top of `cd8bb29` while this entry was being written. Reading them as someone's forgotten leftovers was wrong, and reporting them that way was worse.

**The habit that saved it:** stage files BY NAME, never `git add -A` or `git add .`, and never `git commit -a`. Committing this work touched only its own nine paths, so the concurrent session's in-flight edits were never swept into a stranger's commit and never clobbered. Before writing a session entry here, **`git fetch` and re-read the log** — `docs/CLAUDE.md` may already have newer entries above yours, which is exactly what happened (theirs landed above, both survived).
...
2. **`includeGames` is opt-in per call site. `/events` takes every level; the HOMEPAGE takes varsity only.** Homepage: `getUpcomingEvents(3, { includeGames: true, gameLevels: ["varsity"] })` (2026-08-19). ⚠️ **Never widen that to a bare `includeGames: true`** — all four squads is 44 rows with three or four on the SAME evening, so the three-card strip becomes "Freshmen vs X / JV vs X" essentially forever and buries both the varsity game and the booster events the strip exists to advertise. Varsity alone is ~12 rows across 12 weeks, about one a week, which is what a parent means by "the game".
