...
⚠️ **The live Google Form description is STILL the old "between 12:30 and 1:00 p.m."** — the only volunteer-facing surface not yet fixed, and the one they read *before* signing up. It cannot be fixed from code: `create-coach-meals-form.gs` only runs at form creation, so the description and the date question's help text need a hand-edit in Forms. **Safe to retype both** — nothing reads them. **Never** touch the checkbox option strings (matched byte-for-byte by the website prefill links and `parseDates()`) or add / delete / reorder any question (the automation reads the response sheet by COLUMN POSITION, not header name).
