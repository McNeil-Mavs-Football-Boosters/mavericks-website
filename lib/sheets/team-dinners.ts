...
import { TEAM_DINNERS_SHEET_ID } from "@/lib/constants";
import { TEAM_DINNER_SLOTS } from "@/lib/team-dinners";
import { formatShortName } from "@/lib/sheets/boosters";

/**
 * Read-only access to the Varsity Team Dinner Form-responses sheet.
 *
 * Structurally identical to `lib/sheets/coach-meals.ts` and deliberately kept
 * that way — same service-account JWT (`mcneil-site-reader@…`, sheet shared at
 * Viewer), same first-wins rule, same discriminated-union failure mode. Sheet
 * id is a code constant rather than env because it is not secret.
 *
 ...
 * renders every date as OPEN, and the page would confidently invite ten people
 * to cook dinner for 50 on nights that are already covered.
 *
...
/**
 * A:J, one column wider than coach-meals' A:I. Column G is Cancelled and H/I/J
 * are the three reminder stamps — this signup has a 7-day reminder that the
 * coach-meal pickup does not, because a host has to shop and cook.
 */
const READ_RANGE = `'${SHEET_TAB}'!A:J`;
...
export interface TeamDinnerClaim {
...
export type TeamDinnerSignups =
  | { ok: true; claimsByDate: Map<string, TeamDinnerClaim> }
 ...
 * no option contains a comma — see the warning in lib/team-dinners.ts. Every
...
function parseDateCell(raw: string): string[] {
  ...
  for (const token of tokens) {
    const slot = TEAM_DINNER_SLOTS.find((s) => s.optionText === token);
    if (slot) {
      dates.push(slot.date);
    } else {
      console.error(
        `[team-dinners] Unrecognised date option ${JSON.stringify(token)} in the ` +
          `responses sheet. The form's option text has drifted from ` +
          `lib/team-dinners.ts — prefill links are probably broken too.`,
      );
    }
  }
  return dates;
 ...
 */
export function resolveSignups(rows: string[][]): TeamDinnerSignups {
  const header = rows[0];
  if (!header) {
    // A sheet with no header row is a broken sheet, not an empty one.
    console.error("[team-dinners] Responses sheet has no header row");
  ...
  for (const [key, title] of Object.entries(COL_HEADERS)) {
    ...
    if (found === -1) {
      console.error(
        `[team-dinners] Missing required column ${JSON.stringify(title)}. ` +
  ...
  const claimsByDate = new Map<string, TeamDinnerClaim>();
...
export const getTeamDinnerSignups = cache(async (): Promise<TeamDinnerSignups> => {
  if (TEAM_DINNERS_SHEET_ID.includes("__REPLACE_")) {
    return { ok: false, reason: "TEAM_DINNERS_SHEET_ID is still a placeholder" };
  }

  try {
    const sheets = google.sheets({ version: "v4", auth: buildAuth() });
    const res = await sheets.spreadsheets.values.get({
      spreadsheetId: TEAM_DINNERS_SHEET_ID,
      range: READ_RANGE,
    });
    return resolveSignups((res.data.values ?? []) as string[][]);
  } catch (err) {
    console.error("[team-dinners] Sheet read failed", err);
